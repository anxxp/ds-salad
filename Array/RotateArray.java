...
public class RotateArray {
    ...
    public static void main(String args[]){
        ...
        int numberOfStifts = 3;
        int numberOfElements = input.length;

        ArrayUtility.printArray("INPUT",input);
        rotateArray(input,numberOfStifts,numberOfElements);
        ArrayUtility.printArray("OUTPUT",input);
    }

    private static void rotateArray(int[] input, int numberOfStifts, int numberOfElements) {
        reverse(input,0,numberOfStifts-1);
        reverse(input,numberOfStifts,numberOfElements-1);
